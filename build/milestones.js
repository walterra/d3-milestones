...
function milestones(selector$$1) {
  ...
  function render(data) {
    ...
    var x = time()
      ...
      .domain(extent(allKeys, function (d) { return aggregateFormatParse(d); }));
    ...
    var labelMerge = label.enter().append('div')
      ...
      .classed(cssAboveClass, function (d) { return d.index % 2; });

    var text$$1 = labelMerge.selectAll('.' + cssTextClass).data(function (d) { return [d]; });

    var textEnter = text$$1.enter().append('div')
    ...
    var textMerge = text$$1.merge(textEnter);
      ...
      nestedNodes.forEach(function (d) {
        ...
        nodes.forEach(function (node) {
          ...
          if (
            ...
            var padding = index % 2 ? 'padding-bottom' : 'padding-top';

            // get the height of the next group
            var defaultPadding = 3;
            var nextGroup = nodes[index + 2];
    ...
    timelineMerge.each(function (d, i, node) {
      var margin = 10;
      var maxAboveHeight = max(select(node[i]).selectAll('* .' + cssAboveClass)._groups[0], function (d) { return d.getBoundingClientRect().height; });
      var maxBelowHeight = max(select(node[i]).selectAll('* :not(.' + cssAboveClass + ')')._groups[0], function (d) { return d.getBoundingClientRect().height; });
